...
# Script to launch vertx-pac4j-demo and verify it works
# Usage: ./run_and_check.sh

set -e  # Stop script on error

echo "🚀 Starting vertx-pac4j-demo..."
...
java -jar target/vertx-pac4j-demo-7.0.0-SNAPSHOT-fat.jar > target/server.log 2>&1 &
SERVER_PID=$!
...
if [ "$HTTP_CODE" = "200" ] && [ "$CAS_TEST_PASSED" = "true" ] && [ "$CAS_AUTH_PASSED" = "true" ]; then
    echo "🎉 vertx-pac4j-demo test completed successfully!"
    ...
    echo "💥 vertx-pac4j-demo test failed!"
    if [ "$HTTP_CODE" != "200" ]; then
